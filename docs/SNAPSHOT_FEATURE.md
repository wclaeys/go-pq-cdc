...
func handleMessage(ack replication.Acknowledger, message format.WALMessage) {
    ...
    // CDC events
    case *format.Insert:
        log.Printf("INSERT: %v", msg.TupleData)
    case *format.Update:
        log.Printf("UPDATE: %v -> %v", msg.NewTupleData, msg.OldTupleData)
    case *format.Delete:
        log.Printf("DELETE: %v", msg.OldTupleData)
    }
