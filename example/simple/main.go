...
func Handler(ack replication.Acknowledger, message format.WALMessage) {
	switch msg := message.(type) {
	case *format.Insert:
		slog.Info("insert message received", "new", msg.TupleData)
	case *format.Delete:
		slog.Info("delete message received", "old", msg.OldTupleData)
	case *format.Update:
		slog.Info("update message received", "new", msg.NewTupleData, "old", msg.OldTupleData)
	}

	if err := ack(message.GetLSN()); err != nil {
