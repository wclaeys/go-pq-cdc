...
func TestSnapshotOnlyBasic(t *testing.T) {
	...
	handlerFunc := func(ack replication.Acknowledger, message format.WALMessage) {
...
func TestSnapshotOnlyMultipleTables(t *testing.T) {
	...
	messageCh := make(chan any, 100)
	handlerFunc := func(ack replication.Acknowledger, message format.WALMessage) {
		if snapshot, ok := message.(*format.Snapshot); ok {
			messageCh <- snapshot
...
func TestSnapshotOnlyResume(t *testing.T) {
	...
	handlerFunc1 := func(ack replication.Acknowledger, message format.WALMessage) {
		if snapshot, ok := message.(*format.Snapshot); ok {
			messageCh1 <- snapshot
			if snapshot.EventType == format.SnapshotEventTypeData {
				mu.Lock()
	...
	handlerFunc2 := func(ack replication.Acknowledger, message format.WALMessage) {
		if snapshot, ok := message.(*format.Snapshot); ok {
			messageCh2 <- snapshot
			if snapshot.EventType == format.SnapshotEventTypeData {
				mu.Lock()
