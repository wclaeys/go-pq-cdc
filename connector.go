...
import (
	...
	"github.com/wclaeys/go-pq-cdc/pq/snapshot"
)
...
// nullAcknowledger is a package-level no-op acknowledger used when
// snapshot events are delivered to the listener outside of the normal
// WAL processing flow (there is no meaningful LSN to ack here).
var nullAcknowledger = replication.Acknowledger(func(_ pq.LSN) error {
	return nil
})
...
func (c *connector) snapshotHandler(event *format.Snapshot) error {
	c.listenerFunc(nullAcknowledger, event)
	return nil
}
